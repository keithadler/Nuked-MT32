...
public:
	void clock(uint64_t cycles)
	{
		...
		{
			step = cycles - totalcycles;

			if (step < execute_min_cycles())
				step = execute_min_cycles();
			else if (step > execute_max_cycles())
				step = execute_max_cycles();
	...
	uint64_t total_cycles() { return totalcycles + step - icount; }
...
protected:
	...
	virtual uint32_t execute_min_cycles() const noexcept;
	virtual uint32_t execute_max_cycles() const noexcept;
	...
	void check_irq();
	inline uint8_t read_pc() { return mt32->cpu_read(PC++); }
